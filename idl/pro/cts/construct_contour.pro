FUNCTION construct_contour, n_slice,           $
                            qty,               $
                            n_step,            $
                            tot_steps,         $
                            desc_label,        $ 
  ...
  X               = REFORM(SLB[*,0    ], n1, n2); <- works
  Y               = REFORM(SLB[*,1    ], n1, n2); <- works
...
; Y               = FLTARR((n1+1)*(n2+1))
  ...
  FOR idx = 0, n1-1 DO BEGIN
    FOR jdx = 0, n2-1 DO BEGIN
  
      cur_ndx     = LONG64((idx * (n1)) + jdx)
  ...
  IF (NOT KEYWORD_SET(Q_CNTRS) ) THEN BEGIN
    n_cntrs       = 15
  ENDIF ELSE BEGIN
    size_cntrs    = SIZE(Q_CNTRS,/DIMENSIONS)
    n_cntrs       = size_cntrs[0]
  ENDELSE
 
  CLABS           = INTARR(n_cntrs)

  FOR I = 1, n_cntrs DO BEGIN
  ...
  time            = scan_parameters('tstart', n_step, desc_label, PREFIX=prefix)
...
;  ENDFOR
;  
;  c_label_orient       = INTARR(1,n_cntrs)
;  c_label_orient       = 1
;  
;  c_colors             = IndGen(n_cntrs) + 1
;
...
;
;
;; QSurf                = OBJ_NEW('IDLgrSurface', Q_sqr)
;
