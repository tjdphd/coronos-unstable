   ...
   IF (kth_slc EQ first_slice) THEN BEGIN
      ...
      IF (op_err NE 0) THEN BEGIN
         ...
         ENDIF ELSE BEGIN
                    ...
                    PRINT,  FORMAT = '(a50,1x,a16,1x,a20,1x,a8)',                $
                           'fetch_slices: WARNING - Compressed file not found.', $
                           'Opening the file', FILE_BASENAME(datafile), 'instead.'
      ...
      FOR I = 0, x_res - 1 DO BEGIN
         ...
         ENDFOR
;        PRINT, 'fetch_slices: X[', I, '] = ', X[cur_idx]
      ENDFOR
   ...
   PRINT, FORMAT='(a28,1x,i2,1x,a2,1x,i2,1x,a10,1x,i8,1x,a2,1x,i8,1x,a12,1x,a20,a3)',                $
                 'fetch_slices: fetching slice', line_set, 'of', n3, 'from lines', skip_lines + 1,   $
                 'to', skip_lines + slc_lines, 'of data file', FILE_BASENAME(datafile), '...'
